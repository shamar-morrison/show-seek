import { getGridMetrics, getThreeColumnGridMetrics } from '@/src/utils/gridLayout';
...
describe('gridLayout', () => {
  ...
  it('returns a non-negative item width for two-column layout', () => {
    const metrics = getGridMetrics(393, 2, 16, 24);

    expect(metrics.itemWidth).toBeGreaterThanOrEqual(0);
  });

  it('does not overflow the row width for two-column layout', () => {
    const windowWidth = 393;
    const metrics = getGridMetrics(windowWidth, 2, 16, 24);
    const rowWidth =
      metrics.listPaddingHorizontal * 2 +
      metrics.itemWidth * 2 +
      metrics.itemHorizontalMargin * 2 * 2;

    expect(rowWidth).toBeLessThanOrEqual(windowWidth);
  });

  it('clamps safely for very small widths in two-column layout', () => {
    const metrics = getGridMetrics(8, 2, 16, 24);
