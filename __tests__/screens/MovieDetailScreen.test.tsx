...
let mockMovieLoading = false;

const mockMovie = {
  id: 10,
  title: 'Loaded Movie',
  original_title: 'Loaded Movie',
  overview: 'Loaded Movie Overview',
...
jest.mock('@/src/hooks/useWatchedMovies', () => ({
  useWatchedMovies: () => ({ count: 0, isLoading: false }),
  useAddWatch: () => ({ mutateAsync: jest.fn() }),
  useClearWatches: () => ({ mutateAsync: jest.fn() }),
}));
...
jest.mock('@/src/components/MarkAsWatchedModal', () => () => null);
...
jest.mock('@/src/components/ReminderModal', () => () => null);
...
jest.mock('@/src/components/ui/OpenWithButton', () => ({
  OpenWithButton: () => null,
}));
...
jest.mock('@/src/components/ui/ShareButton', () => ({
  ShareButton: () => null,
}));
...
jest.mock('@/src/components/detail/CollectionSection', () => ({ CollectionSection: () => null }));
jest.mock('@/src/components/detail/DirectorsSection', () => ({ DirectorsSection: () => null }));
jest.mock('@/src/components/detail/ExternalRatingsSection', () => ({
  ExternalRatingsSection: () => null,
}));
jest.mock('@/src/components/detail/MarkAsWatchedButton', () => ({
...
jest.mock('@/src/components/WatchHistoryActionsModal', () => {
  const React = require('react');
  const WatchHistoryActionsModal = React.forwardRef((_props: any, _ref: any) => null);
...
describe('MovieDetailScreen', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockMovieLoading = false;

    mockUseQuery.mockImplementation(({ queryKey }: { queryKey: unknown[] }) => {
      const subKey = queryKey[2];

      if (queryKey[0] === 'movie' && queryKey.length === 2) {
        return {
          data: mockMovieLoading ? undefined : mockMovie,
          isLoading: mockMovieLoading,
          isError: false,
          error: null,
          refetch: jest.fn(),
        };
  ...
  it('does not throw when transitioning from loading to loaded state', () => {
    mockMovieLoading = true;

    const { getByTestId, getByText, rerender } = render(<MovieDetailScreen />);
    expect(getByTestId('detail-skeleton')).toBeTruthy();

    mockMovieLoading = false;

    expect(() => rerender(<MovieDetailScreen />)).not.toThrow();
